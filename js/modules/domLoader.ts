...
// 컴포넌트 로드 함수
async function loadComponent(url: string, targetElement: Element): Promise<boolean> {
    try {
...
async function loadAllComponents(): Promise<void> {
    const componentsToLoad: ComponentConfig[] = [
    ...
    for (const component of componentsToLoad) {
        ...
        if (targetElement) {
            if (component.append) {
                ...
                targetElement.appendChild(tempDiv.firstChild as Node);
            } else {
